...
def _build_total_consumption_series(df: pd.DataFrame) -> pd.Series:
    """Berechnet den gesamten Stromkonsum in kW je Zeitschritt."""
    required_columns = ["load_el_kw", "ely_power_kw", "hp_el_kw", "ev_charge_kw"]
    missing_columns = [col for col in required_columns if col not in df.columns]
    if missing_columns:
        missing = ", ".join(missing_columns)
        raise ValueError(f"DataFrame fehlen Spalten für Stromkonsum: {missing}")

    return df["load_el_kw"] + df["ely_power_kw"] + df["hp_el_kw"] + df["ev_charge_kw"]


def plot_consumption_averages(
    df: pd.DataFrame,
    title: str = "Durchschnittlicher Stromkonsum",
    save_path: str = None,
) -> None:
    """Plottet 3 aggregierte Stromkonsum-Kurven in einem 3x1-Fenster.
    ...
    Args:
        ...
        save_path: Optionaler Pfad zum Speichern des Plots.
    """
    consumption_kw = _build_total_consumption_series(df)

    avg_by_hour = consumption_kw.groupby(consumption_kw.index % 24).mean()

    weekday_order = ["Mo", "Di", "Mi", "Do", "Fr", "Sa", "So"]
    avg_by_weekday = consumption_kw.groupby((consumption_kw.index // 24) % 7).mean()
    avg_by_weekday.index = [weekday_order[int(i)] for i in avg_by_weekday.index]

    avg_by_week = consumption_kw.groupby(consumption_kw.index // 168).mean()
    ...
    axes[0].set_title("Ø Stromkonsum pro Stunde (24h)")
    ...
    axes[1].set_title("Ø Stromkonsum pro Wochentag (Mo-So)")
    ...
    axes[2].set_title("Ø Stromkonsum pro Woche (Jahr)")
...
def plot_consumption_averages_comparison(
    df_base: pd.DataFrame,
    df_optimized: pd.DataFrame,
    title: str = "Durchschnittlicher Stromkonsum Vergleich",
    save_path: str = None,
) -> None:
    """Plottet 3 Vergleichskurven für Base und Optimized in einem 3x1-Fenster.
    ...
    Args:
        ...
        save_path: Optionaler Pfad zum Speichern des Plots.
    """
    base_consumption = _build_total_consumption_series(df_base)
    opt_consumption = _build_total_consumption_series(df_optimized)

    base_avg_by_hour = base_consumption.groupby(base_consumption.index % 24).mean()
    opt_avg_by_hour = opt_consumption.groupby(opt_consumption.index % 24).mean()

    weekday_order = ["Mo", "Di", "Mi", "Do", "Fr", "Sa", "So"]
    base_avg_by_weekday = base_consumption.groupby(
        (base_consumption.index // 24) % 7
    ).mean()
    opt_avg_by_weekday = opt_consumption.groupby(
        (opt_consumption.index // 24) % 7
    ).mean()
    ...
    base_avg_by_week = base_consumption.groupby(base_consumption.index // 168).mean()
    opt_avg_by_week = opt_consumption.groupby(opt_consumption.index // 168).mean()
