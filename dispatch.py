"""
Zentrale Dispatch-Logik fuer alle Strategien.

Die konkreten Strategien liefern nur noch die FC-Entscheidungsregel.
"""
...
def _dispatch_step(
    ...
    """Fuehrt die Energiebilanz fuer einen Zeitschritt aus."""
    dt_h = row['dt_h'] if 'dt_h' in row else 1.0
    pv = row['pv_kw']
    price = row['price_buy']
    load_el = row['load_el_kw']
    ev_home_charge_kw = max(0.0, row['ev_demand_kw'])
    load_heat_kw = row['load_heat_kw']
    load_heat_kwh = load_heat_kw * dt_h
    day_of_year = int(row['day_of_year']) if 'day_of_year' in row else 0

    hp_base_kw = min(load_heat_kw / hp.cop, config.hp_kw_th_max / hp.cop)
    total_el_demand = load_el + ev_home_charge_kw + hp_base_kw
    ...
    if net_el >= 0:
        hp_max_el_kw = config.hp_kw_th_max / hp.cop
        ...
        shortage = abs(net_el)
        if should_use_fc(price, shortage, h2, day_of_year):
            fc_request_kw = min(shortage, config.fc_dispatch_max_kw)
            fc_result = fc.run(fc_request_kw, h2.available_discharge, dt_h=dt_h)
    ...
    if hp_el_adjust_kw >= 0:
        ...
        if should_use_fc(price, add_import_kw, h2, day_of_year):
            extra_fc_request_kw = min(add_import_kw, config.fc_dispatch_max_kw)
            extra_fc_result = fc.run(extra_fc_request_kw, h2.available_discharge, dt_h=dt_h)
    ...
    return {
        ...
        'ev_charge_kw': ev_home_charge_kw,
        'ev_unserved_drive_kwh': 0.0,
        'thermal_soc_kwh': th.soc_kwh,
        'hp_el_kw': hp_el_final_kw,
...
def run_dispatch(profile_df: pd.DataFrame, config: SystemConfig, should_use_fc: ShouldUseFC) -> pd.DataFrame:
    """Fuehrt Dispatch fuer ein gesamtes Profil aus."""
    h2 = H2Storage(config)
    th = ThermalStorage(config)
    ...
    for _, row in profile_df.iterrows():
        results.append(_dispatch_step(row, config, h2, th, ely, fc, hp, should_use_fc))
