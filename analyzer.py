...
def calculate_kpis(
    result_df: pd.DataFrame, config: SystemConfig, label: str = ""
) -> Dict:
    """
    ...
    timestep_hours = (
        result_df["dt_h"]
        if "dt_h" in result_df.columns
        else pd.Series(1.0, index=result_df.index)
    )
    ev_power_column = (
        "ev_charge_kw" if "ev_charge_kw" in result_df.columns else "ev_driven_kwh"
    )
    ...
    total_heat_load_el = (
        (result_df["load_heat_kw"] / config.hp_cop) * timestep_hours
    ).sum()
    total_load_el_equiv = total_el_load + total_heat_load_el

    autarky = (
        max(0.0, 1.0 - (total_import / total_load_el_equiv))
        if total_load_el_equiv > 0
        else 0.0
    )

    costs_import = (
        result_df["grid_import_kw"] * result_df["price_buy"] * timestep_hours
    ).sum()
    costs_export = (
        result_df["grid_export_kw"] * result_df["price_sell"] * timestep_hours
    ).sum()
    net_costs = costs_import - costs_export

    co2_kg = (
        result_df["grid_import_kw"] * result_df["co2_intensity"] * timestep_hours
    ).sum()
    co2_t = co2_kg / 1000.0

    ref_import = total_load_el_equiv
    ref_costs = (ref_import * result_df["price_buy"].mean()) if ref_import > 0 else 0.0
    ref_co2_t = (
        (ref_import * result_df["co2_intensity"].mean()) / 1000.0
        if ref_import > 0
        else 0.0
    )
    ...
    return {
        ...
        "H2-Erzeugung [kWh]": round(
            (result_df["ely_power_kw"] * config.ely_eff_el * timestep_hours).sum(), 0
        ),
    }
...
def save_kpis_to_csv(kpi_list: List[Dict], filepath: str = "kpi_ergebnisse.csv"):
    ...
    resolved_path = (
        os.path.join(output_dir, filepath) if not os.path.isabs(filepath) else filepath
    )
...
def save_kpis_by_scenario(scenario_number: int, kpi_base: Dict, kpi_optimized: Dict):
    ...
    data = {
        ...
        "OptimizedStrategy": [
            kpi_optimized.get(k, "-") for k in kpi_optimized.keys() if k != "label"
        ],
    }
